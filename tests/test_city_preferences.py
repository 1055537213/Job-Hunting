...
def test_profile_stores_multiple_preferred_and_acceptable_cities_separately(tmp_path):
    """新建档案支持多个首选城市，并剔除两个分类中的重复城市。"""

    app = JobHuntingApp(tmp_path / "cities.db")
    app.initialize()
...
def test_conversation_replaces_primary_adds_acceptable_and_can_clear_cities(tmp_path):
    """明确对话会覆盖首选城市、追加可接受城市，并支持城市不限清除。"""

    app = JobHuntingApp(tmp_path / "cities.db")
    app.initialize()
    candidate_id = app.save_candidate_profile(build_profile())

    replaced = app.ingest_conversation_message(candidate_id, "我现在首选上海和苏州。")
    after_replaced = app.get_candidate_profile(candidate_id)
    assert "preferred_cities" in replaced.saved_structured_fields
    assert after_replaced.preferred_cities == ["上海", "苏州"]

    expanded = app.ingest_conversation_message(candidate_id, "首选城市的邻近城市也可以。")
    after_expanded = app.get_candidate_profile(candidate_id)
    ...
    app.ingest_conversation_message(candidate_id, "南京和无锡也可以考虑。")
    after_explicit = app.get_candidate_profile(candidate_id)
    assert {"南京", "无锡"} <= set(after_explicit.acceptable_cities)

    cleared = app.ingest_conversation_message(candidate_id, "现在不用管城市了，任何城市都可以。")
    after_cleared = app.get_candidate_profile(candidate_id)
...
def test_acceptable_city_changes_match_explanation(tmp_path):
...
def test_initialize_adds_acceptable_cities_column_to_old_database(tmp_path):
    """已有 SQLite 档案表会无损补充其他可接受城市列。"""
    ...
    with app.store.connect() as conn:
        columns = {row[1] for row in conn.execute("PRAGMA table_info(candidate_profiles)")}

    assert "acceptable_cities_json" in columns
    assert "preference_weights_json" in columns
